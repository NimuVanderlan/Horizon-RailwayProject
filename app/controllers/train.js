...
const searchTrains = async(req, res)=>{ //UC - View TimeTables.......
	try{
		//check the required fields
		if(!req.query.departureCity || !req.query.arrivalCity || !req.query.date){
			return res.status(400).json({ error: 'Provide the fields'})
		}
		const depStation = await Station.findOne({city :req.query.departureCity});
		const arStation = await Station.findOne({city: req.query.arrivalCity});
		...
		const scheduledRoutes = await ScheduledRoute.find({ //find the matching routes that have been scheduled and populate with the information.
